...
当你没有声明任何构造函数时，编译器通常会隐式声明一个构造函数
...
但如果你声明了其他构造函数，编译器就不会自动生成无参默认构造函数，
...
如果没有自定义析构函数，编译器通常会生成一个，负责按成员声明顺序的逆序销毁成员对象。
...
如果没有自定义拷贝构造函数，编译器通常会生成一个，逐成员拷贝。
...
C++11 引入，用于从临时对象或右值中“移动”资源：

```cpp
A create();
A a = create(); // 可能调用移动拷贝构造
```
...
如果你声明了以下任意一个：
  ...
  编译器通常不会自动生成移动构造函数和移动赋值运算符。

### 3.5. =default 和 =delete
...
关键点在于复制对象时，是否复制对象所指向的资源本身

#### **浅拷贝**

浅拷贝只复制成员变量的值。对于指针成员，只复制地址，不复制指针指向的数据。
...
a.data 和 b.data 指向同一块内存，这就是浅拷贝。
...
一个对象释放资源后，另一个对象持有悬空指针。

#### **深拷贝**

深拷贝不仅复制指针地址，还会重新申请一块资源，并复制原资源中的内容。
